...
function Register() {
  ...
  const form = useForm({
    ...
    validate: {
      ...
      password: (value) =>
        value.length < 7 ? "Password must contain at least 8 characters" : null,
      confirmPassword: (value, values) =>
        value != values.password ? "Passwords do not match" : null,
      username: (value) =>
        value.length < 5 ? "Username must contain at least 6 characters" : null,
      firstName: (value) =>
        value.length === 0 ? "First name cannot be empty" : null,
      lastName: (value) =>
        value.length === 0 ? "Last name cannot be empty" : null,
    },
  });
  ...
  return (
    <form onSubmit={form.onSubmit(handleSubmit)}>
      <ErrorContainer>
        {auth.error.register && (
          <p data-id="register-error" style={{ color: "red" }}>
            {auth.error.register} is taken!
          </p>
        )}
      </ErrorContainer>
      ...
      <FormFieldContainer>
        <TextInput
          data-id="register-username-input"
          sx={{ height: 75 }}
          label="Username"
      ...
      <FormFieldContainer>
        <PasswordInput
          data-id="register-password-input"
          sx={{ height: 75 }}
          label="Password"
      ...
      <FormFieldContainer>
        <PasswordInput
          data-id="register-cfm-password-input"
          sx={{ height: 75 }}
