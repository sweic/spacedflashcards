...
function AppHeaderControl() {
  ...
  const logout = async () => {
    const res = await dispatch(
      apiRequest({ method: "GET", url: "logout", type: "LOGOUT", data: null })
    );
  ...
  return (
    <>
      <Menu
        ...
        control={
          <DeckBtnCircular
            control={<Plus data-id="create-btn" />}
          ></DeckBtnCircular>
        }
      >
        <Menu.Label>Create</Menu.Label>
        <Divider />
        <Menu.Item data-id="import-id-btn" onClick={() => importIDModal.open()}>
          Import by deck ID
        </Menu.Item>
        <Menu.Item
          data-id="new-deck-btn"
          onClick={() => {
            navigate("/u/create", { state: { prevURL: "/u/home" } });
          }}
        >
          New Deck
        </Menu.Item>
      ...
      <Menu
        control={
          <DeckBtnCircular
            control={<Settings data-id="settings-btn" />}
          ></DeckBtnCircular>
        }
      >
