...
export const updateMiddleware: Middleware = storeAPI => next => async action => {
    await next(action)
    ...
    if (action.type.includes('apiSuccess')) {
        switch (action.payload.type) {
            case 'VERIFY':
                ...
                return
            case 'FETCH':
                storeAPI.dispatch(updateData({decks: action.payload.payload.data.decks.decks, dashboardData: action.payload.payload.data.dashboard}))
